var API_URL = `${window.location.href.slice(0, -6)}:7001`;
...
async function getIp() {
    await fetch(`${API_URL}/get_local_ip`)
        .then(response => response.json())
        .then(data => {
            let ip_span = document.querySelector("#ip-span");
            let qrcodeDiv = document.getElementById("qrcode");

            // API_URL = `http://` + data.ip + `:` + `7001`
...
async function uploadFiles() {
    ...
    xhr.upload.addEventListener('progress', (event) => {
        if (event.lengthComputable) {
            ...
            // New value for the conic-gradient
            ...
            // Update the element's style property
